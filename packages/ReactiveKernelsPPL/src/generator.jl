...
# inverts.
# `<family>(remapped args…).logpdf(x)` for a variate expression `x`. Shared by
# scalar priors and the scan (setup + recurrence) density. `gamma` takes rate,
# so the contract's scale inverts.
