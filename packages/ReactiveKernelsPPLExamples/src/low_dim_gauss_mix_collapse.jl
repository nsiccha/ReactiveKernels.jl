...
@kernel model(unconstrained::Vector{Float64},
    ...
    mu1::Float64 = sum(view(unconstrained, 1:1))
    mu2::Float64 = sum(view(unconstrained, 2:2))
...
function evaluate_low_dim_gauss_mix_collapse_source()
    _evaluate_ppl_source(LOW_DIM_GAUSS_MIX_COLLAPSE_SOURCE, @__MODULE__;
        bindings = (:LOW_DIM_GAUSS_MIX_COLLAPSE_Y,))
end
...
function __init__()
    _LOW_DIM_GAUSS_MIX_COLLAPSE_GRAPH_TEMPLATE[] =
        evaluate_low_dim_gauss_mix_collapse_source().model
    nothing
end
