...
# Real data (full) from posteriordb `earnings-log10earn_height`, loaded via PosteriorDB.jl.
let d = _posteriordb_data("earnings-log10earn_height")
    global const LOG10EARN_HEIGHT_EARN = Float64.(d["earn"])
    global const LOG10EARN_HEIGHT_HEIGHT = Float64.(d["height"])
end
...
@kernel model(unconstrained::Vector{Float64},
    ...
    # Likelihood: log10(earnⱼ) ~ Normal(μⱼ, σ). Consumes the named `log10_earn`
    # and `mu` once (single-consumer plate-chain, fused buffer-free).
    pointwise = plate(log10_earn, mu, sigma) do ly, m, s
...
# Raw data BOUND (benchmark-acceptance entry): log10_earn is partial-eval-hoisted.
