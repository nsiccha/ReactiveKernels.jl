using ReactiveKernelsPPLExamples.WellsInteractionExample
using ReactiveKernelsDistributionKernels.DistributionKernelSources: bernoulli
using LogExpFunctions: logistic, log1pexp

# Graph-independent reference oracle for the posteriordb wells_interaction_model:
# a Bernoulli-logit GLM with the three-column design
# x = [dist/100, arsenic, (dist/100)·arsenic], unconstrained parameters
# (identity transform, zero Jacobian) and flat (improper) priors.
function _wells_interaction_reference(q, dist, arsenic, switched)
    alpha, beta1, beta2, beta3 = q[1], q[2], q[3], q[4]
    d100 = dist ./ 100.0
    eta = alpha .+ beta1 .* d100 .+ beta2 .* arsenic .+ beta3 .* (d100 .* arsenic)
...
@testset "PPL graph — wells_interaction (posteriordb)" begin
    artifact = evaluate_wells_interaction_source()
    @test artifact.source == strip(WELLS_INTERACTION_SOURCE, '\n')
    @test artifact.output ==
          Base.invokelatest(artifact.kernel, Tuple(artifact.inputs)...)
    model = artifact.model
    q = [0.1, 0.5, 0.3, -0.2]
    reference = _wells_interaction_reference(q, WELLS_INTERACTION_DIST,
                                             WELLS_INTERACTION_ARSENIC,
                                             WELLS_INTERACTION_SWITCHED)

    @testset "authored on the current baseline surface" begin
        @test occursin("bernoulli(logistic(", WELLS_INTERACTION_SOURCE)
        @test occursin(".logpdf(s)", WELLS_INTERACTION_SOURCE)
        @test occursin("inter = plate(", WELLS_INTERACTION_SOURCE)
        @test occursin("eta = plate(", WELLS_INTERACTION_SOURCE)
        @test occursin("pointwise = plate(", WELLS_INTERACTION_SOURCE)
        @test !occursin("struct ", WELLS_INTERACTION_SOURCE)
    ...
    @testset "posterior decomposition vs the independent reference oracle" begin
        p = plan(model.graph;
                 have = (model.unconstrained, model.dist, model.arsenic, model.switched),
                 want = (model.log_prior, model.pointwise, model.likelihood,
                         model.posterior))
        log_prior, pointwise, likelihood, posterior =
            prepare(p)(q, WELLS_INTERACTION_DIST, WELLS_INTERACTION_ARSENIC,
                       WELLS_INTERACTION_SWITCHED)
    ...
    @testset "generated quantity p from a constrained HAVE prunes the density" begin
        ...
        produced = Set(canon_id(model.graph, o.id)
                       for r in p.recipes for o in r.outputs)
        @test !(canon_id(model.graph, model.likelihood.id) in produced)
        p_values = prepare(p)(reference.parameters, WELLS_INTERACTION_DIST,
                              WELLS_INTERACTION_ARSENIC)
    ...
    @testset "one authored plate exposes a buffer-free total" begin
        ...
        pw = pointwise_kernel(q, WELLS_INTERACTION_DIST, WELLS_INTERACTION_ARSENIC,
                              WELLS_INTERACTION_SWITCHED)
        @test likelihood_kernel(q, WELLS_INTERACTION_DIST, WELLS_INTERACTION_ARSENIC,
                                WELLS_INTERACTION_SWITCHED) ≈ sum(pw)
