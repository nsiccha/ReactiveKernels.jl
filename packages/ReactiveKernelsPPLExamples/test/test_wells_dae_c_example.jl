using ReactiveKernelsPPLExamples.WellsDaeCExample
using ReactiveKernelsDistributionKernels.DistributionKernelSources: bernoulli
using LogExpFunctions: logistic, log1pexp

# Graph-independent reference oracle for the posteriordb wells_dae_c_model:
# a Bernoulli-logit GLM with MEAN-CENTERED distance and arsenic predictors,
# their interaction, and rescaled education. The centering means are computed
# over the observed rows, exactly matching Stan's `mean(dist)` / `mean(arsenic)`
# on the same rows. Unconstrained parameters (identity, zero Jacobian), flat
# priors.
function _wells_dae_c_reference(q, dist, arsenic, educ, switched)
    alpha, beta1, beta2, beta3, beta4 = q[1], q[2], q[3], q[4], q[5]
    ...
    eta = alpha .+ beta1 .* c_dist100 .+ beta2 .* c_arsenic .+
          beta3 .* da_inter .+ beta4 .* educ4
    pointwise = [s == 1 ? -log1pexp(-e) : -log1pexp(e) for (s, e) in zip(switched, eta)]
    likelihood = sum(pointwise)
    (; parameters = (; alpha, beta1, beta2, beta3, beta4), log_jacobian = 0.0,
       log_prior = 0.0, eta, pointwise, likelihood, posterior = likelihood,
       p = logistic.(eta))
end

@testset "PPL graph — wells_dae_c (posteriordb)" begin
    artifact = evaluate_wells_dae_c_source()
    @test artifact.source == strip(WELLS_DAE_C_SOURCE, '\n')
    @test artifact.output ==
          Base.invokelatest(artifact.kernel, Tuple(artifact.inputs)...)
    model = artifact.model
    q = [0.1, 0.5, 0.3, -0.2, 0.15]
    reference = _wells_dae_c_reference(q, WELLS_DAE_C_DIST, WELLS_DAE_C_ARSENIC,
                                       WELLS_DAE_C_EDUC, WELLS_DAE_C_SWITCHED)

    @testset "authored on the current baseline surface" begin
        @test occursin("bernoulli(logistic(", WELLS_DAE_C_SOURCE)
        @test occursin(".logpdf(s)", WELLS_DAE_C_SOURCE)
        @test occursin("sum(dist) / length(dist)", WELLS_DAE_C_SOURCE)
        @test occursin("c_dist100 = plate(", WELLS_DAE_C_SOURCE)
        @test occursin("eta = plate(", WELLS_DAE_C_SOURCE)
        @test occursin("pointwise = plate(", WELLS_DAE_C_SOURCE)
        @test !occursin("struct ", WELLS_DAE_C_SOURCE)
    ...
    @testset "posterior decomposition vs the independent reference oracle" begin
        ...
        log_prior, pointwise, likelihood, posterior =
            prepare(p)(q, WELLS_DAE_C_DIST, WELLS_DAE_C_ARSENIC,
                       WELLS_DAE_C_EDUC, WELLS_DAE_C_SWITCHED)
    ...
    @testset "generated quantity p from a constrained HAVE prunes the density" begin
        ...
        produced = Set(canon_id(model.graph, o.id)
                       for r in p.recipes for o in r.outputs)
        @test !(canon_id(model.graph, model.likelihood.id) in produced)
        p_values = prepare(p)(reference.parameters, WELLS_DAE_C_DIST,
                              WELLS_DAE_C_ARSENIC, WELLS_DAE_C_EDUC)
    ...
    @testset "one authored plate exposes a buffer-free total" begin
        ...
        pw = pointwise_kernel(q, WELLS_DAE_C_DIST, WELLS_DAE_C_ARSENIC,
                              WELLS_DAE_C_EDUC, WELLS_DAE_C_SWITCHED)
        @test likelihood_kernel(q, WELLS_DAE_C_DIST, WELLS_DAE_C_ARSENIC,
                                WELLS_DAE_C_EDUC, WELLS_DAE_C_SWITCHED) ≈ sum(pw)
