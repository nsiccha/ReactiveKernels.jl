using ReactiveKernelsPPLExamples.KidscoreMomhsExample
using ReactiveKernelsDistributionKernels.DistributionKernelSources: normal, cauchy

# Graph-independent reference oracle for the posteriordb kidiq-kidscore_momhs
...
# μ = β₁ + β₂·mom_hs, a half-Cauchy(0, 2.5) prior on sigma, and the new-point
# predicted mean β₁ + β₂·mom_hs_new. Densities are the fully-normalized standard
# forms (matching ReactiveKernels' `normal` and `cauchy` endpoints).
function _kidscore_momhs_reference(q, kid_score, mom_hs, mom_hs_new)
    beta1 = q[1]
    beta2 = q[2]
    log_sigma = q[3]
    sigma = exp(log_sigma)
    log_jacobian = log_sigma
    linpred = beta1 .+ beta2 .* mom_hs
    ...
    (; parameters = (; beta1, beta2, sigma), log_jacobian, linpred,
       ...
       kid_score_pred = beta1 + beta2 * mom_hs_new)
end

@testset "PPL graph — kidscore_momhs (posteriordb)" begin
    artifact = evaluate_kidscore_momhs_source()
    @test artifact.source == strip(KIDSCORE_MOMHS_SOURCE, '\n')
    @test artifact.output ==
          Base.invokelatest(artifact.kernel, Tuple(artifact.inputs)...)
    model = artifact.model
    q = [85.0, 5.0, log(18.0)]
    reference =
        _kidscore_momhs_reference(q, MOMHS_KID_SCORE, MOMHS_MOM_HS, MOMHS_MOM_HS_NEW)

    @testset "authored on the current baseline surface" begin
        @test occursin("normal(", KIDSCORE_MOMHS_SOURCE)
        @test occursin(".logpdf(score)", KIDSCORE_MOMHS_SOURCE)
        @test occursin("cauchy(0.0, 2.5)", KIDSCORE_MOMHS_SOURCE)
        @test occursin("linpred = plate(", KIDSCORE_MOMHS_SOURCE)
        @test occursin("pointwise = plate(", KIDSCORE_MOMHS_SOURCE)
        @test !occursin("struct ", KIDSCORE_MOMHS_SOURCE)
    ...
    @testset "posterior decomposition vs the independent reference oracle" begin
        ...
        log_prior, log_jacobian, pointwise, likelihood, posterior =
            prepare(p)(q, MOMHS_KID_SCORE, MOMHS_MOM_HS)
    ...
    @testset "generated quantity prediction from a constrained HAVE prunes the density" begin
        p = plan(model.graph;
                 have = (model.parameters, model.mom_hs_new),
                 want = (model.kid_score_pred,))
        produced = Set(canon_id(model.graph, o.id)
                       for r in p.recipes for o in r.outputs)
        @test !(canon_id(model.graph, model.likelihood.id) in produced)
        kid_score_pred = prepare(p)(reference.parameters, MOMHS_MOM_HS_NEW)
    ...
    @testset "one authored plate exposes a buffer-free total" begin
        pointwise_kernel = prepare(model;
            have = (:unconstrained, :kid_score, :mom_hs), want = :pointwise)
        likelihood_kernel = prepare(model;
            have = (:unconstrained, :kid_score, :mom_hs), want = :likelihood)
        pw = pointwise_kernel(q, MOMHS_KID_SCORE, MOMHS_MOM_HS)
        @test likelihood_kernel(q, MOMHS_KID_SCORE, MOMHS_MOM_HS) ≈ sum(pw)
