using ReactiveKernelsPPLExamples.KidscoreInteractionExample
using ReactiveKernelsDistributionKernels.DistributionKernelSources: normal, cauchy

# Graph-independent reference oracle for the posteriordb kidiq-kidscore_interaction
...
# μ = β₁ + β₂·mom_hs + β₃·mom_iq + β₄·(mom_hs·mom_iq) (the Stan transformed-data
# interaction), a half-Cauchy(0, 2.5) prior on sigma, and the new-point predicted
# mean. Densities are the fully-normalized standard forms (matching
# ReactiveKernels' `normal` and `cauchy` endpoints).
function _kidscore_interaction_reference(q, kid_score, mom_hs, mom_iq,
...
@testset "PPL graph — kidscore_interaction (posteriordb)" begin
    artifact = evaluate_kidscore_interaction_source()
    @test artifact.source == strip(KIDSCORE_INTERACTION_SOURCE, '\n')
    @test artifact.output ==
          Base.invokelatest(artifact.kernel, Tuple(artifact.inputs)...)
    model = artifact.model
    q = [25.0, -5.0, 0.5, 0.05, log(15.0)]
    reference = _kidscore_interaction_reference(q, INTERACTION_KID_SCORE,
        INTERACTION_MOM_HS, INTERACTION_MOM_IQ,
        INTERACTION_MOM_HS_NEW, INTERACTION_MOM_IQ_NEW)

    @testset "authored on the current baseline surface" begin
        @test occursin("normal(", KIDSCORE_INTERACTION_SOURCE)
        @test occursin(".logpdf(score)", KIDSCORE_INTERACTION_SOURCE)
        @test occursin("cauchy(0.0, 2.5)", KIDSCORE_INTERACTION_SOURCE)
        @test occursin("linpred = plate(", KIDSCORE_INTERACTION_SOURCE)
        @test occursin("pointwise = plate(", KIDSCORE_INTERACTION_SOURCE)
        @test occursin("hs * iq", KIDSCORE_INTERACTION_SOURCE)
        @test !occursin("struct ", KIDSCORE_INTERACTION_SOURCE)
    ...
    @testset "posterior decomposition vs the independent reference oracle" begin
        ...
        log_prior, log_jacobian, pointwise, likelihood, posterior =
            prepare(p)(q, INTERACTION_KID_SCORE, INTERACTION_MOM_HS,
                       INTERACTION_MOM_IQ)
    ...
    @testset "one authored plate exposes a buffer-free total" begin
        ...
        pw = pointwise_kernel(q, INTERACTION_KID_SCORE, INTERACTION_MOM_HS,
                              INTERACTION_MOM_IQ)
        @test likelihood_kernel(q, INTERACTION_KID_SCORE, INTERACTION_MOM_HS,
                                INTERACTION_MOM_IQ) ≈ sum(pw)
