using ReactiveKernelsPPLExamples.KidscoreInteractionC2Example
using ReactiveKernelsDistributionKernels.DistributionKernelSources: normal

# Graph-independent reference oracle for the posteriordb kidiq-kidscore_interaction_c2
...
# μ = β₁ + β₂·c2_mom_hs + β₃·c2_mom_iq + β₄·(c2_mom_hs·c2_mom_iq) uses predictors
# centered on the FIXED reference points (mom_hs - 0.5, mom_iq - 100; Stan's
# transformed data), flat improper priors (log_prior = 0), and the new-point
# predicted mean (centered on the same reference points). Densities are the
# fully-normalized standard forms (matching ReactiveKernels' `normal` endpoint).
function _kidscore_interaction_c2_reference(q, kid_score, mom_hs, mom_iq,
    ...
    linpred = beta1 .+ beta2 .* c2_mom_hs .+ beta3 .* c2_mom_iq .+ beta4 .* inter
    pointwise = [-0.5 * log(2π) - log(sigma) - 0.5 * ((y - m) / sigma)^2
                 for (y, m) in zip(kid_score, linpred)]
    likelihood = sum(pointwise)
    log_prior = 0.0
    c2_hs_new = mom_hs_new - 0.5
    c2_iq_new = mom_iq_new - 100.0
    (; parameters = (; beta1, beta2, beta3, beta4, sigma), log_jacobian, linpred,
       ...
       kid_score_pred = beta1 + beta2 * c2_hs_new + beta3 * c2_iq_new +
                        beta4 * (c2_hs_new * c2_iq_new))
end

@testset "PPL graph — kidscore_interaction_c2 (posteriordb)" begin
    artifact = evaluate_kidscore_interaction_c2_source()
    @test artifact.source == strip(KIDSCORE_INTERACTION_C2_SOURCE, '\n')
    @test artifact.output ==
          Base.invokelatest(artifact.kernel, Tuple(artifact.inputs)...)
    model = artifact.model
    q = [25.0, -5.0, 0.5, 0.05, log(15.0)]
    reference = _kidscore_interaction_c2_reference(q, INTERACTION_C2_KID_SCORE,
        INTERACTION_C2_MOM_HS, INTERACTION_C2_MOM_IQ,
        INTERACTION_C2_MOM_HS_NEW, INTERACTION_C2_MOM_IQ_NEW)

    @testset "authored on the current baseline surface" begin
        @test occursin("normal(", KIDSCORE_INTERACTION_C2_SOURCE)
        @test occursin(".logpdf(score)", KIDSCORE_INTERACTION_C2_SOURCE)
        @test occursin("log_prior::Float64 = 0.0", KIDSCORE_INTERACTION_C2_SOURCE)
        @test occursin("c2_mom_hs = plate(", KIDSCORE_INTERACTION_C2_SOURCE)
        @test occursin("inter = plate(", KIDSCORE_INTERACTION_C2_SOURCE)
        @test occursin("linpred = plate(", KIDSCORE_INTERACTION_C2_SOURCE)
        @test occursin("pointwise = plate(", KIDSCORE_INTERACTION_C2_SOURCE)
        @test occursin("(hs - 0.5) * (iq - 100.0)", KIDSCORE_INTERACTION_C2_SOURCE)
        @test !occursin("cauchy", KIDSCORE_INTERACTION_C2_SOURCE)
        @test !occursin("mean(", KIDSCORE_INTERACTION_C2_SOURCE)
        @test !occursin("struct ", KIDSCORE_INTERACTION_C2_SOURCE)
    ...
    @testset "posterior decomposition vs the independent reference oracle" begin
        ...
        log_prior, log_jacobian, pointwise, likelihood, posterior =
            prepare(p)(q, INTERACTION_C2_KID_SCORE, INTERACTION_C2_MOM_HS,
    ...
    @testset "one authored plate exposes a buffer-free total" begin
        ...
        pw = pointwise_kernel(q, INTERACTION_C2_KID_SCORE, INTERACTION_C2_MOM_HS,
                              INTERACTION_C2_MOM_IQ)
        @test likelihood_kernel(q, INTERACTION_C2_KID_SCORE, INTERACTION_C2_MOM_HS,
                                INTERACTION_C2_MOM_IQ) ≈ sum(pw)
