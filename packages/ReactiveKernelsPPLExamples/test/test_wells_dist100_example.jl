using ReactiveKernelsPPLExamples.WellsDist100Example
using ReactiveKernelsDistributionKernels.DistributionKernelSources: bernoulli
using LogExpFunctions: logistic, log1pexp

# Graph-independent reference oracle for the posteriordb wells_dist100_model:
# a transformed-data rescale dist100 = dist/100, unconstrained parameters with
# flat priors (log prior and log Jacobian both zero), a Bernoulli-logit
# likelihood switched ~ Bernoulli_logit(alpha + beta1*dist100), and the switch
# probabilities p = inv_logit(eta).
function _wells_dist100_reference(q, dist, switched)
    alpha, beta1 = q[1], q[2]
    dist100 = dist ./ 100.0
    eta = alpha .+ beta1 .* dist100
    bern(y, e) = y ? -log1pexp(-e) : -log1pexp(e)
    pointwise = [bern(switched[i], eta[i]) for i in eachindex(switched)]
    likelihood = sum(pointwise)
    (; parameters = (; alpha, beta1), log_prior = 0.0, log_jacobian = 0.0,
       dist100, eta, pointwise, likelihood, posterior = likelihood,
       p = logistic.(eta))
end

@testset "PPL graph — wells_dist100 (posteriordb)" begin
    artifact = evaluate_wells_dist100_source()
    @test artifact.source == strip(WELLS_DIST100_SOURCE, '\n')
    @test artifact.output ==
          Base.invokelatest(artifact.kernel, Tuple(artifact.inputs)...)
    model = artifact.model
    q = [0.1, 0.5]
    reference = _wells_dist100_reference(q, WELLS_DIST100_DIST, WELLS_DIST100_SWITCHED)

    @testset "authored on the current baseline surface" begin
        @test occursin("bernoulli(; logit = e).logpdf(s)", WELLS_DIST100_SOURCE)
        @test occursin(".logpdf(s)", WELLS_DIST100_SOURCE)
        @test occursin("dist100 = plate(", WELLS_DIST100_SOURCE)
        @test occursin("eta = plate(", WELLS_DIST100_SOURCE)
        @test occursin("pointwise = plate(", WELLS_DIST100_SOURCE)
        @test !occursin("struct ", WELLS_DIST100_SOURCE)
    ...
    @testset "transformed data dist100 = dist / 100" begin
        p = plan(model.graph; have = (model.dist,), want = (model.dist100,))
        dist100 = prepare(p)(WELLS_DIST100_DIST)
        @test dist100 ≈ reference.dist100
    end
    ...
    @testset "posterior decomposition vs the independent reference oracle" begin
        ...
        log_prior, pointwise, likelihood, posterior =
            prepare(p)(q, WELLS_DIST100_DIST, WELLS_DIST100_SWITCHED)
    ...
    @testset "generated quantity p from a constrained HAVE prunes the density" begin
        ...
        produced = Set(canon_id(model.graph, o.id)
                       for r in p.recipes for o in r.outputs)
        @test !(canon_id(model.graph, model.likelihood.id) in produced)
        probs = prepare(p)(reference.parameters, WELLS_DIST100_DIST)
    ...
    @testset "one authored plate exposes a buffer-free total" begin
        ...
        pw = pointwise_kernel(q, WELLS_DIST100_DIST, WELLS_DIST100_SWITCHED)
        @test likelihood_kernel(q, WELLS_DIST100_DIST, WELLS_DIST100_SWITCHED) ≈ sum(pw)
