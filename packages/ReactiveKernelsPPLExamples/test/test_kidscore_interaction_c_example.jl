using ReactiveKernelsPPLExamples.KidscoreInteractionCExample
using ReactiveKernelsDistributionKernels.DistributionKernelSources: normal

# Graph-independent reference oracle for the posteriordb kidiq-kidscore_interaction_c
...
# μ = β₁ + β₂·c_mom_hs + β₃·c_mom_iq + β₄·(c_mom_hs·c_mom_iq) uses the MEAN-CENTERED
# predictors (Stan's transformed data), flat improper priors (log_prior = 0), and
# the new-point predicted mean (centered by the SAME training-data means). The
# centering means are computed over the observed rows, exactly matching Stan's
# `mean(mom_hs)` / `mean(mom_iq)` on the same rows. Densities are the
# fully-normalized standard forms (matching ReactiveKernels' `normal` endpoint).
function _kidscore_interaction_c_reference(q, kid_score, mom_hs, mom_iq,
    ...
    c_mom_hs = mom_hs .- mean_hs
    c_mom_iq = mom_iq .- mean_iq
    inter = c_mom_hs .* c_mom_iq
    linpred = beta1 .+ beta2 .* c_mom_hs .+ beta3 .* c_mom_iq .+ beta4 .* inter
    pointwise = [-0.5 * log(2π) - log(sigma) - 0.5 * ((y - m) / sigma)^2
                 for (y, m) in zip(kid_score, linpred)]
    likelihood = sum(pointwise)
    log_prior = 0.0
    c_hs_new = mom_hs_new - mean_hs
    c_iq_new = mom_iq_new - mean_iq
    (; parameters = (; beta1, beta2, beta3, beta4, sigma), log_jacobian, linpred,
       ...
       kid_score_pred = beta1 + beta2 * c_hs_new + beta3 * c_iq_new +
                        beta4 * (c_hs_new * c_iq_new))
end

@testset "PPL graph — kidscore_interaction_c (posteriordb)" begin
    artifact = evaluate_kidscore_interaction_c_source()
    @test artifact.source == strip(KIDSCORE_INTERACTION_C_SOURCE, '\n')
    @test artifact.output ==
          Base.invokelatest(artifact.kernel, Tuple(artifact.inputs)...)
    model = artifact.model
    q = [25.0, -5.0, 0.5, 0.05, log(15.0)]
    reference = _kidscore_interaction_c_reference(q, INTERACTION_C_KID_SCORE,
        INTERACTION_C_MOM_HS, INTERACTION_C_MOM_IQ,
        INTERACTION_C_MOM_HS_NEW, INTERACTION_C_MOM_IQ_NEW)

    @testset "authored on the current baseline surface" begin
        @test occursin("normal(", KIDSCORE_INTERACTION_C_SOURCE)
        @test occursin(".logpdf(score)", KIDSCORE_INTERACTION_C_SOURCE)
        @test occursin("log_prior::Float64 = 0.0", KIDSCORE_INTERACTION_C_SOURCE)
        @test occursin("sum(mom_hs) / length(mom_hs)", KIDSCORE_INTERACTION_C_SOURCE)
        @test occursin("c_mom_hs = plate(", KIDSCORE_INTERACTION_C_SOURCE)
        @test occursin("inter = plate(", KIDSCORE_INTERACTION_C_SOURCE)
        @test occursin("linpred = plate(", KIDSCORE_INTERACTION_C_SOURCE)
        @test occursin("pointwise = plate(", KIDSCORE_INTERACTION_C_SOURCE)
        @test occursin("(hs - mhs) * (iq - miq)", KIDSCORE_INTERACTION_C_SOURCE)
        @test !occursin("cauchy", KIDSCORE_INTERACTION_C_SOURCE)
        @test !occursin("struct ", KIDSCORE_INTERACTION_C_SOURCE)
    ...
    @testset "posterior decomposition vs the independent reference oracle" begin
        ...
        log_prior, log_jacobian, pointwise, likelihood, posterior =
            prepare(p)(q, INTERACTION_C_KID_SCORE, INTERACTION_C_MOM_HS,
    ...
    @testset "one authored plate exposes a buffer-free total" begin
        ...
        pw = pointwise_kernel(q, INTERACTION_C_KID_SCORE, INTERACTION_C_MOM_HS,
                              INTERACTION_C_MOM_IQ)
        @test likelihood_kernel(q, INTERACTION_C_KID_SCORE, INTERACTION_C_MOM_HS,
                                INTERACTION_C_MOM_IQ) ≈ sum(pw)
