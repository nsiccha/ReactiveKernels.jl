...
`alpha'` and `log_lambda[obsyear_i, obssite_i] == alpha[obssite_i]` for ANY year:
the year index provably drops out of the density (`obsyear`/`mis` feed only the
generated-quantities block). The faithful graph therefore binds only the RAW
