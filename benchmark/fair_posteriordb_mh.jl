...
# Fair all-sides benchmark on posteriordb Mh (capture-recapture heterogeneity, dim ~303 — the large-dim scaling test).
#
...
const _INNER = "RK_FAIR_POSTERIORDB_MH_INNER"
const _BODY = joinpath(@__DIR__, "fair_posteriordb_mh_body.jl")

function _run_pinned_comparison()
    root = normpath(joinpath(@__DIR__, ".."))
    mktempdir(prefix = "reactivekernels-fair-posteriordb-mh-") do environment
