# Reactant axes for pandemic_gate.jl. This file is INCLUDED ONLY from the
...
# constants, ReactiveKernels) by being included into the same top level.

function _reactant_axes(name, kb, prep, sm, pts)
    m = match(r"ecdc0\d+", name)
    dataset = m === nothing ? name : m.match
    push!(_REACTANT_DATASETS_RUN, dataset)
    _REACTANT_COVERED_BY[name] = "compiled for dataset $dataset"
