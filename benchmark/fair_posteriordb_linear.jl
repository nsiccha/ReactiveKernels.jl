...
# Fair all-sides benchmark on posteriordb kidscore_interaction + sblri-blr (linear family, completing the 10 named models).
#
...
const _INNER = "RK_FAIR_POSTERIORDB_LINEAR_INNER"
const _BODY = joinpath(@__DIR__, "fair_posteriordb_linear_body.jl")

function _run_pinned_comparison()
    root = normpath(joinpath(@__DIR__, ".."))
    mktempdir(prefix = "reactivekernels-fair-posteriordb-linear-") do environment
