...
const NUTS_INTERACTION = raw"""
using LinearAlgebra
using Random

RK = ReactiveKernels
fixture = Main.ReactiveKernelsDocsNUTSFixture
prepared = RK._prepare_factory(
    fixture.euclidean_phasepoint,
    RK.kernel_registration(fixture.leapfrog!),
)
plan = RK.kernel_prepared_plan(prepared)
metric = [2.0 0.0; 0.0 2.0]
endpoint_values = Dict{Int,Any}()
for slot in RK.kernel_plan_slots(plan)
    name = String(slot.path[1])
    endpoint_values[slot.canon] = name == "pot_f" ?
        (position -> sum(abs2, position)) : name == "grad_f" ?
        ((gradient, position) -> (gradient .= 2 .* position; sum(abs2, position))) :
        name == "metric" ? metric : name == "chol_metric" ? cholesky(metric) :
        startswith(name, "##node") ? 0.0 : name == "pos" ? [1.0, 2.0] :
        name == "mom" ? [3.0, 4.0] :
...
entry = only(ReactiveKernels.method_irs(fixture.walnuts!!))
receipt_path = joinpath(
    pkgdir(ReactiveKernels), "benchmark", "receipts",
    "walnuts-upstream-macro-v1.tsv",
)
receipt_lines = readlines(receipt_path)
receipt_rows = filter(line -> !startswith(line, '#'), receipt_lines)[2:end]
length(state_methods) == 15 && all(method -> method.ok, state_methods) ||
    error("WALNUTS fixture no longer has its admitted 15-method boundary")
map(formal -> (formal.name, formal.kind, formal.required), entry.formals) == (
    (:momentum, :kw, true),
    (:directions, :kw, true),
...
docs_interaction = (;
    ...
    output = (;
        state_kernel_kind = state_registration.kind,
        captured_method_count = length(state_methods),
