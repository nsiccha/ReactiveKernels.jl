module WalnutsKernelAuthoringFixture

# WALNUTS-D compiler fixture.  This is mathematical source, not package API.
#
...
@kernel walnuts_state(init; step_f, macro_time,
    ...
    reset!() = begin
        ...
        exponential_index = 0
        replay_overflow = false
        fill!(attempted_micro_steps, 0)
        fill!(forward_attempts, 0)
        fill!(reverse_checks, 0)
        fill!(macro_accepted, false)
        copy!!(fwd, init)
        copy!!(bwd, init)
        ...
        for _ in 1:max_step_halvings
            ...
            if abs(candidate.ham - ep.ham) <= max_error
                ...
                while reverse_num_steps >= 2 * min_micro_steps
                    ...
                    reverse_stepsize *= 2
                    copy!!(reverse_candidate, candidate)
    ...
    step!(directions, exponentials) = begin
        ...
        copy!!(init, proposals[length(proposals)])
    end
