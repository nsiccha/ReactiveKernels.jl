...
# Fair all-sides benchmark on posteriordb gp_regr (Gaussian-process regression, structured covariance + cholesky).
#
...
const _INNER = "RK_FAIR_POSTERIORDB_GP_INNER"
const _BODY = joinpath(@__DIR__, "fair_posteriordb_gp_body.jl")

function _run_pinned_comparison()
    root = normpath(joinpath(@__DIR__, ".."))
    mktempdir(prefix = "reactivekernels-fair-posteriordb-gp-") do environment
